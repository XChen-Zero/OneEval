...
: "${ENABLE_THINKING_1:?Need env ENABLE_THINKING_1 (true|false|none)}"
: "${ENABLE_THINKING_2:?Need env ENABLE_THINKING_2 (true|false|none)}"

# =========================
# Optional envs (defaults)
# =========================
PYTHON_BIN="${PYTHON_BIN:-python}"
TRACK_1="${TRACK_1:-thinking}"
TRACK_2="${TRACK_2:-non_thinking}"
...
FILTER_THINKING_1="${FILTER_THINKING_1:-true}"
FILTER_THINKING_2="${FILTER_THINKING_2:-false}"

# How long to wait for server readiness
WAIT_RETRIES="${WAIT_RETRIES:-120}" # 120 * 2s = 240s
...
echo "[INFO] BASE_PATH=${BASE_PATH} MODEL_NAME=${MODEL_NAME} TRACK_1=${TRACK_1} TRACK_2=${TRACK_2}"
echo "[INFO] ENABLE_THINKING_1=${ENABLE_THINKING_1} FILTER_THINKING_1=${FILTER_THINKING_1}"
echo "[INFO] ENABLE_THINKING_2=${ENABLE_THINKING_2} FILTER_THINKING_2=${FILTER_THINKING_2}"

EVAL_ARGS_1=(
  --base-path "${BASE_PATH}"
  --model-name "${MODEL_NAME}"
  --track "${TRACK_1}"
  --enable-thinking "${ENABLE_THINKING_1}"
)

case "${FILTER_THINKING_1,,}" in
  1|true|yes|y)
    EVAL_ARGS_1+=(--filter-thinking)
    ;;
esac
...
EVAL_ARGS_2=(
  ...
  --model-name "${MODEL_NAME}"
  --track "${TRACK_2}"
  --enable-thinking "${ENABLE_THINKING_2}"
)

case "${FILTER_THINKING_2,,}" in
  1|true|yes|y)
    EVAL_ARGS_2+=(--filter-thinking)
    ;;
esac
...
echo "[INFO] Evaluation Thinking completed successfully!"

"${PYTHON_BIN}" "${EVAL_SCRIPT}" "${EVAL_ARGS_2[@]}"
